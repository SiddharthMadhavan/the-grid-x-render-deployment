...
def main() -> None:
    """Main entry point. Uses PORT (Railway/Render) or GRIDX_HTTP_PORT for single-port deployment."""
    # Railway/Render set PORT; use it for both HTTP and WebSocket (same server)
    port = int(os.getenv("PORT") or os.getenv("GRIDX_HTTP_PORT", "8080"))

    print("=" * 60)
    print("🌐 Grid-X Coordinator - FIXED VERSION 1.0.0")
    print("=" * 60)
    print(f"📡 HTTP + WebSocket: http://0.0.0.0:{port}")
    print(f"   - HTTP API:  /jobs, /credits, /workers, /health")
    print(f"   - WebSocket: /ws/worker")
    print("=" * 60)
    print(f"💡 Workers connect to: https://<your-domain>:{port}/ws/worker")
    print("   (Railway: use your app URL without port, e.g. wss://xxx.up.railway.app/ws/worker)")
    print("=" * 60)
